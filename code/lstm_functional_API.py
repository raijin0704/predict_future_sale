...
def model_study(hidden_depth, hidden_layers, batch_size, hidden_activation, model_name, data_name, y_standard_F, item_dim, shop_dim, item_category_dim):

    train_x, train_y, test_x, test_y, test_y_label, test_y_mean, test_y_std, train_y_ori, test_y_ori, train_info, test_info = load_all_numpy_data(data_name)
    train_item = train_info[:,0]
    train_shop = train_info[:,1]
    ...
    file_name = data_name + "_" + model_name + "_" + str(hidden_depth)+ "depth_" + str(hidden_layers) + "layers_"  + hidden_activation + "-activation_" + "dim" + str(item_dim) + "i" + str(shop_dim) + "s" + str(item_category_dim) + "c"
    if y_standard_F:
        pass
    else:
    ...
    shop_input = Input(shape=(1,), dtype="int32", name="shop_input")
    ...
    info_dense = Dense(hidden_layers)(concatenated_info)
    ...
    model = Model([series_input, item_input, shop_input, item_category_input], output)
    ...
    if y_standard_F:
        history = model.fit({"series_input":train_x, "item_input":train_item, "shop_input":train_shop, "item_category_input":train_item_category}, {"model_output":train_y}, batch_size=batch_size, epochs=100, validation_split=0.2, callbacks=[mc_cb,es_cb])
    else:
        history = model.fit({"series_input":train_x, "item_input":train_item, "shop_input":train_shop, "item_category_input":train_item_category}, {"model_output":train_y_ori}, batch_size=batch_size, epochs=100, validation_split=0.2, callbacks=[mc_cb,es_cb])
    with open("../result/other/history/" + file_name + ".pickle", mode="wb") as f:
        pickle.dump(history.history, f)

    test_y_pre = model.predict({"series_input":test_x, "item_input":test_item, "shop_input":test_shop, "item_category_input":test_item_category})
...
model_name = "functional_dense2depth_LSTM"
...
for hidden_layers in [16,32,64]:
    model_study(1, hidden_layers, batch_size, hidden_activation, model_name, data_name, y_standard_F, 100, 10, 10)