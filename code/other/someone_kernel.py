...
if VALID is True:
    ...
    callbacks_list=[EarlyStopping(monitor="val_loss",min_delta=.001, patience=3,mode='auto'), ModelCheckpoint("other/output/model.hdf5", save_best_only=True)]
    ...
    plt.legend()
    plt.show()
    plt.savefig("other/output/Train and Validation MSE Progression.png")
...
if VALID is False:
    ...
    plt.legend()
    plt.show()
    plt.savefig("other/output/Training Loss Progression.png")

print("\nOutput Submission")
submission = pd.DataFrame(pred,columns=['item_cnt_month'])
submission.to_csv('other/output/submission.csv',index_label='ID')
