...
def main_func(model_name, mode):
    ...
    Arguments:
        model_name {String} -- 予測モデルの名前(submissionやtestのcsvファイル名)
...
def make_submission_plot(test_table, model_name, submission_path):
    ...
    plot_path = "./graph/submit/"+model_name+"/"
...
def make_test_plot(test_table, model_name, test_path):
    ...
    plot_path = "./graph/test/"+model_name+"/"
    if not os.path.exists(plot_path):
        os.makedirs(plot_path)
...
if __name__ == "__main__":
    main_func("simple_data_5mfilter_functional_easy_dense1depth_LSTM_1depth_64layers_relu-activation_y_ori", "t")